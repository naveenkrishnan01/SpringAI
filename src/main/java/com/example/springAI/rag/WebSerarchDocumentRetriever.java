...
public class WebSerarchDocumentRetriever implements DocumentRetriever {

    private static final Logger logger = LoggerFactory.getLogger(WebSerarchDocumentRetriever.class);
    ...
    public WebSerarchDocumentRetriever(RestClient.Builder clientBuilder, int resultLimit) {
        Assert.notNull(clientBuilder, "clientBuilder cannot be null");
        String apiKey = System.getenv(TAVILY_API_KEY);
        if (apiKey == null || apiKey.isBlank()) {
            logger.warn("Environment variable {} is not set. Web search functionality will be disabled.", TAVILY_API_KEY);
    ...
    public List<Document> retrieve(Query query) {
        ...
        if (restClient == null) {
            logger.warn("RestClient is not initialized (missing API key). Skipping retrieval.");
            return List.of();
        }
    ...
    public static class Builder {
        ...
        public WebSerarchDocumentRetriever build() {
            return new WebSerarchDocumentRetriever(clientBuilder, resultLimit);
        }
    }
}